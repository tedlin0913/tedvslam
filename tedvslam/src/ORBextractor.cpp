...
#undef GET_VALUE
    ...
    static bool isFastCorner(cv::Mat &img, cv::KeyPoint &keypoint, int threshold)
    {
        ...
        for (i = -255; i <= 255; i++)
            threshold_tab[i + 255] = (uchar)(i < -threshold ? 1 : i > threshold ? 2
                                                                                : 0);
    ...
    static void computeOrientation(const cv::Mat &image, std::vector<KeyPoint> &keypoints, const std::vector<int> &umax)
    {
                                                              ...
                                                              const int &maxX, const int &minY, const int &maxY, const int &N, const int &level)
    {
        ...
        while (!bFinish)
        {
            ...
            while (lit != lNodes.end())
            {
                if (lit->bNoMore)
                {
                    // If node only contains one cv::Point do not subdivide and continue
                    lit++;
                    continue;
                }
                else
                {
                    // If more than one cv::Point, subdivide
            ...
            // or all nodes contain just one cv::Point
        ...
        // Retain the best cv::Point in each node
    ...
    static void computeDescriptors(const cv::Mat &image, std::vector<KeyPoint> &keypoints, cv::Mat &descriptors,
    ...
    void ORBextractor::DetectAndCompute(cv::InputArray _image, cv::InputArray _mask, std::vector<KeyPoint> &_keypoints,
    ...
    void ORBextractor::Detect(cv::InputArray _image, cv::InputArray _mask, std::vector<KeyPoint> &_keypoints)
    {
    ...
    void ORBextractor::ScreenAndComputeKPsParams(cv::InputArray _image, std::vector<KeyPoint> &_keypoints,
                                                 std::vector<cv::KeyPoint> &out_keypoints)
    {
        ...
        for (size_t i = 0, N = _keypoints.size(); i < N; i++)
        {
            ...
            if (!(kps.pt.y - EDGE_THRESHOLD >= 0 && kps.pt.y + EDGE_THRESHOLD < imgPyramid.rows && kps.pt.x - EDGE_THRESHOLD >= 0 && kps.pt.x + EDGE_THRESHOLD < imgPyramid.cols))
            {
                kps.pt *= scale;
                continue;
            }
    ...
    void ORBextractor::DetectWithPyramid(cv::InputArray _image, cv::InputArray _mask, std::vector<KeyPoint> &_keypoints)
    {
        if (_image.empty() || _mask.empty())
            return;
    ...
    void ORBextractor::CalcDescriptors(cv::InputArray _image, const std::vector<KeyPoint> &_keypoints,
                                       cv::OutputArray _descriptors)
    {
        ...
        // Pre-compute the scale pyramid
        ComputePyramid(image);
        // prepare the blurred working cv::Mat
    ...
    void ORBextractor::ComputePyramid(cv::Mat image, cv::Mat mask)
    {
        ...
        for (int level = 0; level < nlevels; ++level)
        {
            ...
            if (level != 0)
            {
                ...
                cv::resize(mvMaskPyramid[level - 1], mvMaskPyramid[level], sz, 0, 0, INTER_LINEAR);
            }
        }
    }
